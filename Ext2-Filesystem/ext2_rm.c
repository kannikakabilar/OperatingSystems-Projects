...
removing the specified file from the disk. If the file does not exist or if it is a directory, then the program returns 
the appropriate error. When a file or link is removed (e.g., no need to zero out data blocks, but it must set i_dtime 
in the inode, removing a directory entry need not shift the directory entries after the one being deleted).
BONUS: Implement an additional "-r" flag (after the disk image argument), which allows removing directories as well. 
In this case, it recursively removes all the contents of the directory specified in the last argument. If "-r" is used 
with a regular file or link, then it will be ignored (the ext2_rm operation will be carried out as if the flag had not 
been entered).
*/
#include <stdio.h>
#include <stdlib.h>
