...
and the second is an absolute path to a file or link (not a directory!) on that disk. The program is the 
exact opposite of rm, restoring the specified file that has been previous removed. If the file does not 
exist (it will be overwritten), or if it is a directory, then the program returns the appropriate error.
Since, the file to be restored will not appear in the directory entries of the parent directory, unless it 
searches the "gaps" left when files get removed. The directory entry structure is the key to finding out 
these gaps and searching for the removed file. If the directory entry for the file has not been overwritten, 
it will still make sure that the inode has not been reused, and that none of its data blocks have been reallocated. 
However, the bitmaps are reliable indicators of such fact. If the file cannot be fully restored, the program will 
terminate with ENOENT, indicating that the operation was unsuccessful. 
While there are some removed entries already present in some of the image files provided, the respective files 
have been removed on a non-ext2 file system, which is not doing the removal the same way that ext2 would. In ext2, 
when "rm" is performed , the inode's i_blocks do not get zeroed, and it can do full recovery. In other FSs things 
work differently. In ext3, when rm a file is performed, the data block indexes from its inode do get zeroed, 
so recovery is not as trivial. For example, there are some removed files in deletedfile.img, which have their blocks zero-ed out 
(due to how these images were created). In such cases, the code will still work, but simply recover a file as an empty file 
(with no data blocks).
Note(3): We will not try to recover files that had hardlinks at the time of removal. This is because when trying to restore a file, 
if its inode is already in use, there are two options: the file we're trying to restore previously had other hardlinks (and hence 
its inode never really got invalidated), _or_ its inode has been re-allocated to a completely new file. Since there is no way to 
tell between these 2 possibilities, recovery in this case should not be attempted.
An additional "-r" flag is implemented (after the disk image argument), which allows restoring directories as well. In this case, 
it will have recursively restore all the contents of the directory specified in the last argument. If "-r" is used with a regular 
file or link, then it will be ignored (the restore operation will be carried out as if the flag had not been entered).
